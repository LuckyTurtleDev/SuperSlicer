...
static boost::filesystem::path find_file(boost::filesystem::path filename) {
    ...
    if (!boost::filesystem::exists(filename)) {
        ...
        size_t count = readlink("/proc/self/exe", result, _MAX_PATH);
...
static int run_script(const std::string &script, const std::string &gcode, std::string &/*std_err*/)
{
    ...
    std::cout << (boost::format("Executing script : %1%") % boost::nowide::narrow(command_line)) << "\n";
...
static int run_script(const std::string &script, const std::string &gcode, std::string &std_err)
{
    std::cout << boost::format("run_script : $") << script << "$\n";
    ...
    std::cout << boost::format("first_space = %1% ? %2%") % first_space % std::string::npos<<"\n";
    ...
    std::cout << boost::format("command script : %1%") % command;
    if (boost::iends_with(command, L".pl")) {
        ...
        std::cout << boost::format("Executing script : detecting python script") << "\n";
        // This is a python script. Run it through the python interpreter.
        command_line = "python ";
    }
    ...
    std::cout << (boost::format("Executing script, shell: %1%, command: %2%") % shell % command_line) << "\n";
